...
class OreFinderApp:
    ...
    def _apply_origin(self, save, cache):
        """Default the coordinate origin to the world spawn (= map middle for a
        world with no custom spawn), so app coordinates match the in-game
        position readout. This is the original, correct behaviour."""
        sp = cache.get("spawn") if cache else None
        if not sp:
            return
